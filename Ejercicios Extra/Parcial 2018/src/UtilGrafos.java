...
public class UtilGrafos {
    ...
    public static Double[][] obtenerMatrizCostos(IGrafoDirigido grafo) {
        return obtenerMatrizCostos(grafo.getVertices());
    }
    ...
    public static <T extends IGrafoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        List<TVertice> verticesList = new ArrayList<>(vertices.length);
        List<TArista> aristasList = new ArrayList<>(aristas.length);

        for (String verticeString : vertices) {
            if ((verticeString != null) && (!verticeString.trim().isEmpty())) {
                verticeString = verticeString.split(",")[0];
                verticesList.add(new TVertice(verticeString));
            }
        }
        for (String aristaString : aristas) {
            if ((aristaString != null) && (!aristaString.trim().isEmpty())) {
