...
public class TGrafoDirigido implements IGrafoDirigido {
    ...
    /*
    Precondiciones:
        Los vértices del grafo dirigido almacenan valores de tipo Integer.
     */
    public TCamino caminoCritico() {
        Map<Comparable, Integer> pesoCritico = new HashMap();
        Map<Comparable, TAdyacencia> sucesorCaminoCritico = new HashMap();
        
        int max = 0;
        TVertice origen = null;
        for (TVertice vertice : this.vertices.values()) {
            int value = caminoCritico(origen, pesoCritico, sucesorCaminoCritico);
            if (max < value) {
                max = value;
                origen = vertice;
            }
        ...
        while (sucesorCaminoCritico.containsKey(origen)) {
            TAdyacencia ady = sucesorCaminoCritico.get(origen);
            res.agregarAdyacencia(ady);
            origen = ady.getDestino();
        }
        return res;
    }

    private static int caminoCritico(TVertice vertice,
            Map<Comparable, Integer> pesoCritico,
            Map<Comparable, TAdyacencia> sucesorCaminoCritico) {
        {
            Integer res = pesoCritico.get(vertice.getEtiqueta());
            if (res != null) {
                return res;
            }
        }
        ...
        int max = 0;
        TAdyacencia sucesor = null;
        for (Object ob : vertice.getAdyacentes()) {
            TAdyacencia ady = (TAdyacencia) ob;
            int value = caminoCritico(ady.getDestino(), pesoCritico, sucesorCaminoCritico);
            if (value > max) {
                max = value;
                sucesor = ady;
            }
        }
        max += (Integer) vertice.getDatos();
        
        pesoCritico.put(vertice.getEtiqueta(), max);
        if (sucesor != null) {
            sucesorCaminoCritico.put(vertice.getEtiqueta(), sucesor);
        }
        return max;
    }
